...
import type { CrossSection, LaneDirection, LaneSpec, Way } from "./system";
 ...
 *  right half forward (a single directional lane becomes bidirectional). */
export function makeTwoWay(profile: CrossSection): CrossSection {
  ...
  const directionByLaneId = new Map(travel.map((l, i) => [l.id, (i < half ? "backward" : "forward") as LaneDirection]));
  return {
 ...
 *  keep the split balanced; removing takes from the majority side. */
export function withLaneCount(profile: CrossSection, typeId: string, count: number): CrossSection {
  ...
  while (primaries().length < target) {
    ...
    const anchor = direction === "backward" ? lanes.indexOf(primaries()[0]) : lanes.indexOf(primaries()[primaries().length - 1]) + 1;
 ...
 * Returns null when the profile is already one-way (nothing to separate).
 */
export function separateProfiles(profile: CrossSection): SeparatedProfiles | null {
  ...
  return { backward: oneWay(leftLanes, "backward"), forward: oneWay(rightLanes, "forward") };
}

/** Join two one-way carriageway profiles back into one two-way profile with
 *  a median between them. `backward` lanes come first (left half). */
export function combineProfiles(backward: CrossSection, forward: CrossSection, medianWidthM?: number): CrossSection {
