...
export interface WorkbenchProps {
  ...
  menuPanel: ReactNode;
  /** Selection details — null when nothing's selected. Desktop docks it as
   *  its own card on the right; mobile swaps the sheet over to it. */
 ...
 */
export function Workbench({ loadError, brand, menuPanel, supplementalPanel, hasSelection, primaryToolbar, viewSwitcher, modeToolbar }: WorkbenchProps) {
  const [sheetExpanded, setSheetExpanded] = useState(false);
  const clearSelection = useEditor((s) => s.select);

  useEffect(() => {
    if (hasSelection) setSheetExpanded(true);
  }, [hasSelection]);

  const showingSupplemental = hasSelection;
  ...
  return (
    <>
      ...
      >
        ...
        {showingSupplemental && (
          <button type="button" className="sheet-back" onClick={() => clearSelection(null)}>
